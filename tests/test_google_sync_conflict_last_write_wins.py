...
class _RepoConflictStub:
    def __init__(self):
        self.task = TaskRecord(
            id="task-1",
            case_id="case-1",
            case_ref="db://cases/case-1",
            title="Cambio local mas reciente",
            due_date="2026-03-10",
            updated_at="2026-03-10T15:00:00+00:00",
        )
        self.mapping = GoogleEventMap(
            ...
            last_local_updated_at="2026-03-10T15:00:00+00:00",
        )
        self.synced_token = ""
...
def test_google_sync_conflict_prefers_latest_local_update(monkeypatch) -> None:
    repo = _RepoConflictStub()
    ...
    monkeypatch.setattr(sync_mod, "upsert_event", lambda *args, **kwargs: {"id": "ev-1", "updated": "2026-03-10T16:00:00Z", "etag": "etag-local-push"})

    def _list_events(service, calendar_id, sync_token="", page_token=""):
        event = {
            "id": "ev-1",
            "summary": "Cambio remoto viejo",
            "description": "Google",
    ...
    assert result["pulled_updates"] == 0
